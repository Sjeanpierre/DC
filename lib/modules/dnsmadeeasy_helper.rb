module DnsmadeeasyHelper
  ...
  class Dns
      ...
      class DnsResult < Struct.new(:subdomain_name, :dns_id, :ip_address)
      end
      ...
      def self.create_record(domain, record)
        new_record = @dns_client.create_record!(domain, record)
        delete_record(domain, new_record[:id]) # don't forget to remove this
        DnsResult.new(new_record[:name], new_record[:id], new_record[:data])
      end
